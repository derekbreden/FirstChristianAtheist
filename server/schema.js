      ...
      if (found_root_user.rows.length) {
        ...
        for (const page_path of page_paths) {
          ...
          if (found_page.rows.length) {
            page.topic_id = found_page.rows[0].topic_id;
            if (page.body !== found_page.rows[0].body) {
              await client.query(
                ...
                WHERE topic_id = $2;
                `,
                [page.body, found_page.rows[0].topic_id],
              );
            }
          } else {
            const new_page = await client.query(
              `
              INSERT INTO topics
                (title, slug, body, user_id, parent_topic_id, admin)
              VALUES
                ($1, $2, $3, $4, 0, true)
              RETURNING topic_id;
              `,
            ...
            page.topic_id = new_page.rows[0].topic_id;
          }
        }
      }
    } catch (err) {
